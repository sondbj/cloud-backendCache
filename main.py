...
cache_collection = get_database() # storing the function to get the cache db in a variable
...
def get_contacts(update_cache=False):
    cache_key = 'contacts'
    security_key = request.headers.get('X-Security-Key')  # Get the security key from the request headers

    if validate_security_key(security_key):  # Implement your security key validation logic
        if update_cache:
        ...
        if cache_entry:
            ...
            if last_accessed is not None:
                ...
                cache_collection.update_one({"_id": cache_key}, {"$set": {"last_accessed": datetime.now()}})
        else:
            backend_endpoint = '/contacts'
            response = requests.get(backend_api_url + backend_endpoint)
            data = response.json()

            cache_collection.insert_one({"_id": cache_key, "data": data, "last_accessed": datetime.now()})
...
def get_by_id_vcard(id):
    ...
    if validate_security_key(security_key):  # Implement your security key validation logic
        ...
        if cache_entry:
            ...
            cache_collection.insert_one({"_id": cache_key, "data": vcard_content, "last_accessed": datetime.now()})
...
#FUNKER IKKE FORELØPIG

@app.route('/cache/upload.html', methods=['POST'])
def cache_upload_vcard():
    security_key = request.form.get('securityKey')  # Retrieve the security key from the form data

    if not validate_security_key(security_key):  # Implement your security key validation logic
        return jsonify({'error': 'Invalid security key'}), 401
...
def get_all_contacts_vcard():
    ...
    if validate_security_key(security_key):  # Implement your security key validation logic
        ...
        if cache_entry:
            ...
            if response.status_code == 200:
                vcards = response.content  # Assuming the backend response is the vCard content
                cache_collection.insert_one({"_id": cache_key, "data": vcards, "last_accessed": datetime.now()})
            else:
